<template>
  ...
  >
    <div class="f-column h-100">
      <div class="custom-header f-row w-100 f-noshrink" :class="headerCls" :style="headerStyle">
        <!-- header -->
        <slot name="header">
      ...
      <!-- body -->
      <div class="custom-body f-full f-column" :class="bodyCls" :style="bodyStyle">
        <overlay-scrollbars-vue class="f-full h-100 os-host-flexbox">
          <slot name="body"> </slot>
        </overlay-scrollbars-vue>
      </div>

      <!-- footer -->
      <div class="custom-footer f-noshrink" :class="footerCls" :style="footerStyle">
        <slot name="footer">
...
export default {
  ...
  props: {
    ...
    closeBtnText: { type: String, default: '취소' },
  },
  data() {
    return {
      isDialogClosed: true,
  ...
  computed: {
    computedDialogStyle() {
      return Object.assign({ width: '75vw', height: '80vh', maxHeight: '80vh' }, this.dialogStyle)
    },
  },
  methods: {
    close() {
